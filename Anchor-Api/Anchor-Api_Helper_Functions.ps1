# Functions that are used by other functions, and not exported to the powershell session. (i.e. Not for user use)

# Testing Functions

Function Update-AnchorModule{
    [Alias('ReloadAnchor')]
    param()
    $currentOauthToken = Get-AnchorOauthToken
    If($currentOauthToken){
        $currentOauthToken | Export-Clixml $env:USERPROFILE\AnchorOauthToken.xml
    }
    Else{
        $currentOauthToken = Import-Clixml $env:USERPROFILE\AnchorOauthToken.xml
    }
    Import-Module Anchor-Api -Force
    $currentOauthToken | Set-AnchorOauthToken
}

Function Get-AnchorRawData {
    param(
    ...
    try{
        $results = Invoke-RestMethod -Uri "$Global:apiUri`/$ApiEndpoint" -Method $Method -Headers $headers -Body $body
    }
    catch{
    ...
    If ($results.PSobject.Properties.name -eq "results") { # The returned object contains a property named "results" and is therefore a collection. We have to do some magic to extract all the data. 
        $results # Return the first set of results.
        ...
        While ($resultsCount -lt $totalResults -and $resultsCount -lt $ResultsLimit){ # Keep calling the endpoint until we've squeezed out all the data.
            ...
            $results # Return the next batch of results.
            $resultsCount+=$results.results.count
        }
    ...
    begin{
        ...
        $scriptblock = {
            Param (
                ...
                [string]$ApiEndpoint,
                [hashtable]$ApiQuery,
                $Tag, # Whatever is passed here will be sent back with the results. It's not passed with the query, just saved for later
                [int]$PageLimit
            )
            ...
            If ($results.PSobject.Properties.name -eq "results") { # The returned object contains a property named "results" and is therefore a collection. We have to do some magic to extract all the data. 
                # Add the tag and return the first set of results.
                $myResults = $results.results
                # At least one function returns an array of string objects instead of a pscustomobject, so we have to turn it into a pscustomobject before we can attach the tag property and return it.
                If($myResults[0].GetType().Name -eq 'string'){
                ...
                # "Normal" results just get the tag added on.
                Else{
                ...
                for ($offset=100; $offset -le (100 * $additionalCallCount); $offset+=100){
                    ...
                    $myResults # Return the next set of results.
                    Write-Verbose "Invoke-RestMethod for $apiUri`/$ApiEndpoint complete at $(Get-Date)"
                }

            } Else { #This is an object (or empty). We can just add the tag and return the results.
